...
public class Minimax {
	...
	public static int alphaBeta(int alpha,int beta,long WP,long WN,long WB,long WR,long WQ,long WK,long BP,long BN,long BB,long BR,long BQ,long BK,long EP,boolean CWK,boolean CWQ,boolean CBK,boolean CBQ,boolean WhiteToMove,int depth) {//using fail soft with negamax
        int bestScore;
        int bestMoveIndex = -1;
        if (depth == Game.searchDepth)
        {
        ...
        if (firstLegalMove == -1)
        {
            return WhiteToMove ? Game.MATE_SCORE : -Game.MATE_SCORE;
        }
        ...
        bestScore = -alphaBeta(-beta,-alpha,WPt,WNt,WBt,WRt,WQt,WKt,BPt,BNt,BBt,BRt,BQt,BKt,EPt,CWKt,CWQt,CBKt,CBQt,!WhiteToMove,depth+1);
        Game.moveCounter++;
        if (Math.abs(bestScore) == Game.MATE_SCORE)
        {
            return bestScore;
        }
        if (bestScore > alpha)
        ...
        for (int i=firstLegalMove+4;i<moves.length();i+=4) {
            ...
            if ((score > alpha) && (score < beta))
            {
                ...
                bestScore = -alphaBeta(-beta,-alpha,WP,WN,WB,WR,WQ,WK,BP,BN,BB,BR,BQ,BK,EP,CWK,CWQ,CBK,CBQ,!WhiteToMove,depth+1);
                if (score>alpha)
                {
                    bestMoveIndex = i;
                    alpha = score;
            ...
            if ((score != Game.NULL_INT) && (score > bestScore))
            {
                if (score >= beta)
                {
                    return score;
